...
export function StepRevisionFinanzas({ step, pimId, stageKey, pim, userId, userName, userRole, userDepartment }: Props) {
        ...
        <div className="flex items-center gap-2 text-sm">
          {resultado === 'aceptada' ? (
            <>
              ...
              <span className="text-green-700">Corrección aceptada por Finanzas</span>
            </>
          ) : (
            <>
              <XCircle className="h-4 w-4 text-red-600" />
              <span className="text-red-700">Corrección rechazada — devuelta para nueva subsanación</span>
            </>
          )}
        </div>
      ...
      },
      {
        onSuccess: () => {
          supabase.from('pim_activity_log').insert({
            ...
            stage_key: stageKey,
            tipo: 'nc_accepted',
            descripcion: `Finanzas aceptó la subsanación de NC ${nc.codigo}`,
            usuario: userName,
            usuario_id: userId,
          ...
          completeStep.mutate(
            {
              ...
              pimId,
              stageKey,
              stepKey: 'revision_finanzas',
              stepName: 'Revisión Finanzas',
              userId,
              userName,
              datos: { nc_id: nc.id, resultado: 'aceptada' },
            },
            {
              onSuccess: () => toast.success('Corrección aceptada. Continúa con Registro Banco y Tasa.'),
      ...
      },
      {
        onSuccess: () => {
          supabase.from('pim_activity_log').insert({
            ...
            stage_key: stageKey,
            tipo: 'nc_rejected',
            descripcion: `Finanzas rechazó la subsanación de NC ${nc.codigo}: ${motivoRechazo}`,
            usuario: userName,
            usuario_id: userId,
                ...
                if (users && users.length > 0) {
                  ...
                  supabase.from('notificaciones').insert(
                    users.map((u) => ({
                      ...
                      mensaje: `Finanzas rechazó la subsanación: ${motivoRechazo.slice(0, 100)}`,
                      leido: false,
                      prioridad: 'alta',
                      fecha_creacion: now,
                    }))
          ...
          reactivateStep.mutate(
            ...
            {
              onSuccess: () => {
                toast.info('Corrección rechazada. Devuelta al área responsable para nueva subsanación.');
    ...
    <div className="space-y-4">
      ...
      {showRejectForm && (
        <div className="space-y-3 p-4 bg-red-50/50 border border-red-200 rounded-lg">
          ...
          <Textarea
            rows={2}
            placeholder="Explique por qué la corrección no es aceptable..."
            value={motivoRechazo}
      ...
      {isFinanzas && nc.estado !== 'resuelta' && (
        <div className="text-sm text-muted-foreground flex items-center gap-2">
          <Clock className="h-4 w-4" />
          Esperando que el área asignada envíe la subsanación...
        </div>
      )}

      {!isFinanzas && (
        <div className="text-sm text-muted-foreground">
          Esperando revisión de Finanzas sobre la subsanación presentada.
        </div>
      )}
    </div>
  );
