...
export function StepDeclaracionNCFin({ step, pimId, stageKey, pim, userId, userName, userRole }: Props) {
  ...
  const handleCrearNC = () => {
    ...
    createNC.mutate(
      ...
      {
        onSuccess: (nc) => {
          // Propagate nc_id to steps 4 (subsanacion_nc_fin) and 5 (revision_finanzas)
          if (allSteps) {
            const step4 = allSteps.find((s) => s.step_key === 'subsanacion_nc_fin');
            const step5 = allSteps.find((s) => s.step_key === 'revision_finanzas');
            if (step4) {
              updateStepData.mutate({ stepId: step4.id, pimId, stageKey, datos: { nc_id: nc.id, nc_codigo: nc.codigo } });
            }
            if (step5) {
              updateStepData.mutate({ stepId: step5.id, pimId, stageKey, datos: { nc_id: nc.id, nc_codigo: nc.codigo } });
            }
          }
          ...
          completeStep.mutate(
            {
              ...
              pimId,
              stageKey,
              stepKey: 'declaracion_nc_fin',
              stepName: 'Declaración de NC',
              userId,
              userName,
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-3 p-4 bg-yellow-50/50 border border-yellow-200 rounded-lg">
        ...
        <div>
          <Label className="text-xs">Descripción de la observación *</Label>
          <Textarea
            className="mt-1"
            rows={3}
            placeholder="Describa el detalle de la observación financiera..."
            value={ncDescripcion}
